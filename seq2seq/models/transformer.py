...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        i) Without positional embeddings, the model would degrade to a Bag-of-Word model, whereas a language model 
        based on Markov chain would perform better according to the previous experience in NLP. Thus, adding the
        positional embeddings enable the model to follow the distributional hypothesis, i.e. able to learn the 
        conditional probability of a token given its context.
        Considering the context information for calculationg $h_t$, LSTM is more like an autogressive model as $h_1, h_2, \dots, h_{t-1}$ are all involved, whereas Transformaer is more like an $n$-order Markov model where $n$ is the number of tokens in the inputs.

        ii) In LSTM, the hidden/cell state $h_i / c_i$ are calculated from $\tilde{c}_i$ and $c_{i-1}$. For $c_{i-1}$,
        it was then also calculated from $c_{i-2}$, and so on. Thus, the positional information are actually implicitly
        encoded by LSTM during its forward computation.
        '''
...
class TransformerDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out=None, incremental_state=None, features_only=False):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            i) The architecture of the Transformer make it possible for decoders to just copy the tokens from the 
            follow-ups, which cannot be done during prediction. To solve this problem, we could use self_attn_mask to 
            hide the future tokens when training the decoder.

            ii) For incremental decoding, we generate the token one by one. Thus, the available tokens for generating 
            $y_i$ are $\{y_1, y_2, \dots, y_{i-1}\}$, and there is no token from the future. In this case, we do not 
            need to mask to hide the future information.
            '''
        ...
        if not features_only:
            ...
            i) This layer is used to select a word in the vocabulary given the hiddent states in the model. It works
            because the CrossEntropyLoss criterion combines nn.LogSoftmax() and nn.NLLLoss() in one single class.

            ii) The forward_state would have the same dimensionality to the size of vocabulary.
            
            iii) If features_only=True, then the output would the features (hidden states) generated by the model, just
