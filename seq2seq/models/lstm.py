...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            self.lexical_context_proj = nn.Linear(embed_dim, embed_dim, bias=False)
            self.lexical_final_proj = nn.Linear(embed_dim, len(dictionary))
            # TODO: --------------------------------------------------------------------- /CUT
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        for j in range(tgt_time_steps):
            ...
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    lexical_contexts.append(torch.tanh(self.lexical_context_proj(lexical_context)) + \
                                                       lexical_context)
                    # TODO: --------------------------------------------------------------------- /CUT
        ...
        if self.use_lexical_model:
            ...
            decoder_output += self.lexical_final_proj(lexical_contexts)
            # TODO: --------------------------------------------------------------------- /CUT
